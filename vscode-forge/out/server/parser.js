...
class ForgeParser {
    ...
    parse(text, uri) {
        ...
        while (this.pos < this.text.length) {
            ...
            if (this.isDeclarationKeyword(word)) {
                const result = this.parseDeclaration(word, startLine, startColumn);
                if (result) {
    ...
    parseBlock(kind, parentName, references) {
        const fields = [];
        this.advance(); // skip {
        let braceCount = 1;
    ...
    parsePathExpression(references) {
...
function getWordAtPosition(text, line, character) {
    ...
    return {
        word,
        range: vscode_languageserver_1.Range.create(vscode_languageserver_1.Position.create(line, start), vscode_languageserver_1.Position.create(line, end))
    };
}
