...
const parser = new parser_1.ForgeParser();
const symbolTable = new symbols_1.SymbolTable();
const completionProvider = new completion_1.CompletionProvider(symbolTable);
// Workspace folders
let workspaceFolders = [];
connection.onInitialize((params) => {
    // Store workspace folders
    if (params.workspaceFolders) {
        workspaceFolders = params.workspaceFolders.map(f => vscode_uri_1.URI.parse(f.uri).fsPath);
    }
    else if (params.rootUri) {
        workspaceFolders = [vscode_uri_1.URI.parse(params.rootUri).fsPath];
    }
...
function scanWorkspaceForForgeFiles() {
...
function generateDiagnostics(document, result) {
    const diagnostics = [];
    // Add parse errors
    for (const error of result.errors) {
        diagnostics.push({
            severity: error.severity === 'error' ? node_1.DiagnosticSeverity.Error :
...
connection.onCompletion((params) => {
    ...
    if (!document)
        return [];
    return completionProvider.getCompletions(document.getText(), params.position.line, params.position.character);
});
...
connection.onDefinition((params) => {
    ...
    if (!document)
        return null;
    const wordInfo = (0, parser_1.getWordAtPosition)(document.getText(), params.position.line, params.position.character);
    if (!wordInfo)
        return null;
    ...
    if (parts.length >= 2) {
        ...
        if (entityDef) {
            ...
            if (field) {
                return node_1.Location.create(field.uri, field.nameRange);
            }
...
connection.onDocumentSymbol((params) => {
    ...
    return symbols.map(symbol => {
        ...
        return {
            name: symbol.name,
            kind: symbols_1.SymbolTable.toLspSymbolKind(symbol.kind),
            range: symbol.range,
...
connection.onWorkspaceSymbol((params) => {
    ...
    return symbols
        ...
        name: symbol.name,
        kind: symbols_1.SymbolTable.toLspSymbolKind(symbol.kind),
        location: node_1.Location.create(symbol.uri, symbol.nameRange)
    }));
});
// Hover
connection.onHover((params) => {
    ...
    if (parts.length >= 2) {
        ...
        if (field) {
            ...
            return { contents: { kind: node_1.MarkupKind.Markdown, value: detail } };
        }
    }
    // Type hover info
    const typeHovers = {
